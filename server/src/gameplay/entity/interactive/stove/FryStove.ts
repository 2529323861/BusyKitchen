  ...
  public interactiveCallback(entity: GamePlayerEntity) {
    ...
    switch (state) {
      case StoveState.StoveIdleState: {
        ...
        if (JsonDataMgr.instance.searchFryTable(playerItem.id)) {
          /** 玩家手中的东西可以煎 */
          /** 将玩家手中的东西放入炉子中 */
          ...
          /** 状态切换煎炸中 */
          console.log('(server): 玩家用可交互物品交互闲置煎锅');
      ...
      case StoveState.CookingFinishState: {
        ...
        if (playerItem.id === '1001') {
          ...
          const product = JsonDataMgr.instance.getDateFromItemMap(
            (
              JsonDataMgr.instance.searchFryTable(
                this.storageItem.id
              ) as IFryTableData
            ).product
          );
          ...
          console.log('(server): 玩家将结束煎锅中的产物取出');
          /** 切换状态为闲置状态 */
          this._stateMachine!.transitionTo(StoveState.StoveIdleState);
        } else {
          /** 玩家手中不为空，不可取出物品 */
          console.log('(server): 玩家无法取出结束煎锅');
          /** 预留UI接口 */
        }
        break;
      }
      case StoveState.BurntPotState: {
        ...
        if (playerItem.id === '1001') {
          ...
          this.storageItem = JsonDataMgr.instance.getDateFromItemMap('1000');
          console.log('(server): 玩家将锅灰从烧糊煎锅中取出');
          this._stateMachine!.transitionTo(StoveState.CleaningState);
        } else {
          /** 玩家手中不为空，不可取出物品 */
          console.log('(server): 玩家无法取出烧糊煎锅');
          /** 预留UI接口 */
        }
        break;
      }
