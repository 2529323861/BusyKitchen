...
export class StateMachine {
  ...
  public init(): void {
    if (this.states.size >= 1) {
      this.currentState = this.states.get(this.config.initialState) ?? null;
   ...
   */
  public addState(state: IState, isInit: boolean = false): void {
    console.log('(Server) StateMachine addState', state.name);
    ...
    if (
      isInit &&
      this.states.size === 1 &&
      state.name === this.config.initialState
    ) {
  ...
  public transitionTo(
    newStateName: string,
    isSkipCheck: boolean = false
  ): boolean {
    // Check if the new state exists
    const newState = this.states.get(newStateName);
    if (!newState) {
      console.error(
        `State ${newStateName} does not exist in the state machine`
      );
      return false;
    }
    ...
    if (!isSkipCheck && this.currentState) {
      const allowedTransitions =
        this.config.transitions[this.currentState.name];
      if (!allowedTransitions || !(newStateName in allowedTransitions)) {
        console.error(
          `Transition from ${this.currentState.name} to ${newStateName} is not allowed`
        );
        return false;
      }
    }
  ...
  public checkTransitions(): boolean {
    ...
    for (const transition of this._curPossibleTransitions) {
      const shouldTransition = transition.conditions.every((condition) =>
        condition.check()
      );
      if (shouldTransition) {
